...
**(local)**
```bash
git push -u origin deploy/railway-setup
```
Review the diff on GitHub, then merge it into `main` (or deploy straight
from this branch — Railway can point at any branch, and you can switch it
to `main` later once you're happy).
...
2. **New Project** → **Deploy from GitHub repo** → pick `Sanchez-313/Medical-Project`, authorize Railway if prompted.
3. Railway will try to auto-detect one service from the repo root — that becomes your **web** service. Rename it to something clear, e.g. `azuremed-web`, in its Settings tab.
...
Railway sets `PORT` and builds/starts the app automatically (`npm run
build` / `npm run start`, both already in `package.json`) — no changes
needed there.
...
2. In its **Settings**, set **Root Directory** to `bot`. Railway will detect
   Python from `bot/requirements.txt` and `bot/Procfile` (`worker: python
   bot.py`) — no build command needed.
